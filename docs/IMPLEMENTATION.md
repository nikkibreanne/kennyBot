...
**in the nikkibreanne.github.io repo** (not here).
...
> **Status (read this first).** This doc was written *before* the build, so §G and
> §H are the original **plan**, not a description of the code — both are now
> implemented, and the real layout is in the README's Architecture section. The
> deployment, secrets, rules and combat sections (§A–§F, §I–§L) are still current
> and are what the source's `§` markers point at.
>
> It predates the clip pipeline entirely — for anything touching `!clip`, local
> capture or Aitum, [`clip-architecture.md`](clip-architecture.md) is authoritative.
> Game-balance tunables are in [`CONFIG.md`](CONFIG.md).
...
```
merge to main ──► release-please ──► tag ──► build + push ──► GHCR (private)
                                                             │  authenticated pull
                                                             ▼
...
`.github/workflows/publish.yml` — **the sketch below is the original plan.** As
built, releases are cut by **release-please** from Conventional Commits and the
image is published from `release.yml`; `publish.yml` now only serves a
hand-pushed `v*` tag, and both call a shared reusable workflow that runs the full
test suite first. See the README's *Releasing* section for the current flow.
...
- **Health.** No HTTP endpoint exists, so a port probe is meaningless. The image
  ...
  snapshot `{ts, version, chatConnected, live, sendMode, clipMode}` to `HEARTBEAT_FILE` on each activity tick,
...
- Use a **distroless** runtime (`gcr.io/distroless/nodejs22-debian12:nonroot`) —
